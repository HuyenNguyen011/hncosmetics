...
function pl_fail_php_version() {
    ...
    $message = sprintf(esc_html__('Post Layouts for Gutenberg requires PHP version %s+, plugin is currently NOT RUNNING.', PL_DOMAIN), '5.6');
...
add_filter('block_categories', function( $categories, $post ) {
    return array_merge(
        ...
        array(
            'slug' => 'post-layouts',
            'title' => __('Post Layouts By Techeshta', PL_DOMAIN),
        ),
            )
    );
}, 10, 2);
